...
import { BaseState, CompositionState, State } from "../scripts/state.js";
import { transform2d, transform2dMap } from "../scripts/transforms.js";
...
const STATE = JSON.parse(localStorage?.getItem("state") ?? "null") ?? {
    ...
    gridState: new Map([]) as CellularAutomatonTypes.State
};
...
/// Clearing Screen
function clearScreen(ctx: CanvasRenderingContext2D, fade = false) {
    ctx.save();
    ctx.setTransform(1, 0, 0, 1, 0, 0);
    if (fade) {
...
function colorCursorAndText(ctx: CanvasRenderingContext2D, mouseX: number, mouseY: number) {
...
async function pattern(fileName: string): Promise<CellularAutomatonTypes.State> {
    return new Map(await fetch(`/patterns-json/${fileName}.json`).then(res => res.json()));
}
...
(async function main() {
    ...
    function evolveState(state: CellularAutomatonTypes.State, generations: number = 1): CellularAutomatonTypes.State {
        let tempGrid = new CellularAutomaton(
            state,
    ...
    /// Gosper Glider Gun
    const gosperGliderGun = await pattern("gosperglidergun");

    /// Pentadecathlon
    const pentadecathlon = await pattern("pentadecathlon");

    /// Buckaroo
    const buckaroo = await pattern("buckaroo");

    /// Glider Eater
    const gliderEater = await pattern("eater1");
    ...
    function gunEaterCombo(distance = 0) {
        return [
            ...
            State.base(
                gliderEater,
                [32 + distance, - 6 - distance]
            )
        ]
    }

    /// Not Gate
    function notGate(translation = [0, 0], gliderGunEvolveAmount = 5, ...transforms: [(map: CellularAutomatonTypes.State) => CellularAutomatonTypes.State]) {
        return State.base(
            State.compile(
                [
                    State.base(
    ...
    /// And Gate
    function andGate(translation = [0, 0], gliderGunEvolveAmount = 0, eaterDistance = 0, ...transforms: [(map: CellularAutomatonTypes.State) => CellularAutomatonTypes.State]) {
        return State.base(
            transforms.reduce(
                (acc, cur) => cur(acc),
                State.compile(
                    [
                        ...
                        State.base(
                            gliderEater
                            ,
                            [23 + eaterDistance, - 4 - eaterDistance]
                        )
                    ]
                )
            ),
    ...
    /// Or Gate
    function orGate(translation = [0, 0], gliderGunEvolveAmount = 0, secondGliderDistance = 0, secondGliderGunEvolveAmount = 0, ...transforms: [(map: CellularAutomatonTypes.State) => CellularAutomatonTypes.State]) {
        return State.base(
            transforms.reduce(
                (acc, cur) => cur(acc),
                State.compile(
                    [
                        ...
                        State.base(
                            evolveState(
                                transform2dMap.flipX(
                                    gosperGliderGun
                                ),
                                secondGliderGunEvolveAmount
                            )
                            ,
                            [23 + secondGliderDistance + 36 - 30, - 4 - secondGliderDistance - 36 + 34]
                        ),
                        State.base(
                            transform2dMap.flipX(
                                gliderEater
                            ...
                            ,
                            [23 + secondGliderDistance + 36 - 30 - 31, - 4 - secondGliderDistance - 36 + 34]
                        )
                    ]
                )
            ),
    ...
    // Compiling State
    const compiledStartingState = finalState.compile() as CellularAutomatonTypes.State;

    // Setting up Config
    config.startingState = compiledStartingState;

    const grid: CellularAutomaton = new CellularAutomaton(
        compiledStartingState,
        config.cellInspectorFunction,
        config.evolverFunction,
        config.cellGetterFunction
    );
    ...
    async function loop() {
        ...
        grid.evolveState();
        draw(ctx);
        await delay(STATE.delayPerFrame);
...
window.addEventListener("keydown", (ev) => {
    ...
    switch (keyPressed) {
        case "Space":
            ...
            break;
        default:
            if (IMPORTANT_KEYS.includes(keyPressed)) STATE.keyIsPressed[ev.code] = true;
            break;
    }
});
...
window.addEventListener("keyup", (ev) => {
    let keyPressed = ev.code;
    if (IMPORTANT_KEYS.includes(keyPressed)) STATE.keyIsPressed[ev.code] = false;
});
...
setInterval(async () => {
    ...
    if (redrawCanvas) setupCanvas(ctx);
